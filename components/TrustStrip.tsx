...
function AvatarList({ ariaHidden = false }: { ariaHidden?: boolean }) {
  return (
    <div
      className='flex w-max flex-none items-start gap-x-8 pr-8'
...
export function TrustStrip() {
  return (
    <section
      id='trust-strip'
      className='lg:mt-16 px-6 pb-16 text-center md:pb-20'
    >
      <p className='mb-5 font-sans text-base font-bold text-black/60'>
        Trusted by contractors all over the US
      </p>
      <div className='relative mx-auto max-w-[500px] overflow-hidden [mask-image:linear-gradient(to_right,transparent,black_10%,black_90%,transparent)] [-webkit-mask-image:linear-gradient(to_right,transparent,black_10%,black_90%,transparent)]'>
        <div className='marquee-track flex w-max items-start'>
          {/* Set 1 of 2 (duplicated below for a seamless loop — see
              globals.css for how the marquee animation uses this).
